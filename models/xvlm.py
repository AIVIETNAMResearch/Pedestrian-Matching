...
class XVLMBase(nn.Module):
    ...
    def load_pretrained(self, ckpt_rpath, config, is_eval=False, is_domain_pretrain=False, use_mlm_loss=False):
        ...
        if is_domain_pretrain:
            ...
            state_dict = load_pretrained(self, ckpt_rpath, config, is_eval=is_eval, load_text=True, use_mlm_loss=use_mlm_loss)
    ...
    def get_sdm_loss(self, image_feat, text_feat, pid=None, epsilon=1e-8):
        """
        ...
        image_norm = image_feat_all / image_feat_all.norm(dim=1, keepdim=True)
        text_norm = text_feat_all / text_feat_all.norm(dim=1, keepdim=True)

        t2i_cosine_theta = text_norm @ image_norm.t()
        i2t_cosine_theta = t2i_cosine_theta.t()

        text_proj_image = t2i_cosine_theta / self.temp
        image_proj_text = i2t_cosine_theta / self.temp
